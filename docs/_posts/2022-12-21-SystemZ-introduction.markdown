...
我设定这个游戏的货币系统为 $Z$ , 1单位的 $Z$ 表示 1小时的工作时间，可以换取 $x$ 单位的休息时间：

$$
Z = 1\ (hour \ work) = x\ (free \ time)
$$
...
$Z$ 的赚取必须通过完成每日任务/通行证奖励/常务任务奖励得到，并规定
...
1. 任务：3小时研究；奖励：$3Z$
2. 任务：1.5小时论文阅读（至少看完一篇）；奖励：$2 Z$
3. 任务：1.5小时书籍学习（至少看完两节）；奖励：$2 Z$
4. 任务：1小时课外书；奖励：$1 Z$
5. 任务：1小时技术博客；奖励：$1 Z$

以上是初始每日任务的设定。除了每日任务外，系统也鼓励玩家自行探索。也即完成以上的 1-3 项，玩家以 0.9 倍的工作时间得到 $Z$。上课、课堂作业、会议等必做事项也可以 0.8 的倍率得到 $Z$。

以上所列的工作时间，均需要通过 forest 软件记录。时间段（30-60分钟）中间可以有小段休息（15分钟，或60分钟吃饭时间），休息超出时间限制必须对之前的时间段减半处理。

### 通行证系统

在通行证中，玩家可以累计一定数量的积分 $H$ 来达到通行证的奖励要求，并领取奖励。目前得到 $H$ 的方法暂定为以下两种：

- 每次通过任务获取 $Z$ ，即可赚取等量的 $H$.
- 一天内完成所有的每日任务，额外获得 $3H$.
